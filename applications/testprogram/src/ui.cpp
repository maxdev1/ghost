...
void run_test(int argc, char** argv) {
	...
	if (hasPause) {
		int time;
		pause >> time;
		std::cout << "pausing for " << time << "ms before attempting to open UI"
				<< std::endl;
		if (time >= 0 && time < 30000) {
			g_sleep(time);
		}
	} else {
		std::cout << "no pause requested (" << argc << " args supplied)"
				<< std::endl;
	}
	...
	if (open_stat == G_UI_OPEN_STATUS_SUCCESSFUL) {
		...
		klog("creating the window took %i ms", g_millis() - start);
