...
public:
	 ...
	 *
	 */
	terminal_t(bool headless) :
			headless(headless), screen(0), shell_in(G_FD_NONE), shell_out(G_FD_NONE), shell_err(G_FD_NONE), echo(true), input_mode(G_TERMINAL_MODE_DEFAULT), current_process(
					-1) {
	}
