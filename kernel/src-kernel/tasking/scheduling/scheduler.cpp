 ...
 *
 */
g_scheduler::g_scheduler(uint32_t coreId) : milliseconds(0), coreId(coreId), currentRound(0), run_queue(0), idle_entry(0), current_entry(0), high_priority(0)
{
}
...
void g_scheduler::updateMilliseconds()
{
	...
	if (milliseconds - last_debugout_millis > 10000)
	{
		...
		while (entry)
		{
			...
			g_log_info("%# running - %i:%i, eip: %h, waiter: %s, name: %s, rounds: %i", thr->process->main->id, thr->id, thr->statePtr->eip,
 ...
 *
 */
g_thread *g_scheduler::save(g_processor_state *statePtr)
{
	if (!current_entry) return schedule();
	current_entry->value->statePtr = statePtr;
	return current_entry->value;
}
...
void g_scheduler::selectNext()
{
	...
	if (run_queue == 0)
	{
		...
		if (current_entry == 0)
		{
			kernelPanic("%! idle thread does not exist on core %i", "scheduler", coreId);
		}
		return;
	}
	...
	// schedule fairly
	g_task_entry *next = run_queue;
	g_task_entry *oldest = 0;
	uint64_t oldestTime = 0;

	while (next)
	{
		if (next->value->schedulerRound != currentRound && (oldest == 0 || milliseconds - next->value->lastRunTime > oldestTime))
		{
			oldest = next;
			oldestTime = milliseconds - next->value->lastRunTime;
		}
		next = next->next;
	}

	current_entry = oldest;
	if (current_entry == 0)
	{
		current_entry = idle_entry;
	}
	current_entry->value->lastRunTime = milliseconds;
	current_entry->value->schedulerRound = currentRound;
}

g_task_entry *g_scheduler::findEntry(g_thread *thread)
{

	g_task_entry *next = run_queue;
	while (next)
	{
		if (next->value == thread)
		{
			return next;
		}
	}
 ...
 *
 */
g_thread *g_scheduler::schedule(g_thread *next)
{

	currentRound++;

	g_thread *preferred = next;
	...
	while (true)
	{
		if (preferred)
		{
			current_entry = findEntry(preferred);
			if (current_entry == nullptr)
			{
				g_log_info("%! warning: tried to schedule to thread which is not present in scheduler", "scheduler");
				current_entry = idle_entry;
			}
			preferred = nullptr;
		}
		...
		// check waiters
		if (_checkWaitingState(current_entry->value))
		{
			continue;
		}
 ...
 *
 */
bool g_scheduler::_checkWaitingState(g_thread *thread)
{
