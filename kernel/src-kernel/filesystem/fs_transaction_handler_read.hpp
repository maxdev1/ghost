...
#ifndef GHOST_FILESYSTEM_TRANSACTION_HANDLER_READ
#define GHOST_FILESYSTEM_TRANSACTION_HANDLER_READ
 ...
 *
 */
class g_fs_transaction_handler_read: public g_fs_transaction_handler {
public:
	g_fs_transaction_handler_read(g_fs_node* node, g_file_descriptor_content* fd, g_contextual<g_syscall_fs_read*> data) :
			node(node), fd(fd), data(data) {
	}

	g_fs_node* node;

	g_fs_read_status status = G_FS_READ_ERROR;
	int64_t result = -1;
	g_file_descriptor_content* fd;
	g_contextual<g_syscall_fs_read*> data;
