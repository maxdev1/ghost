...
#ifndef GHOST_INTERRUPTS_IOAPIC
#define GHOST_INTERRUPTS_IOAPIC
...
public:
	g_ioapic(uint32_t id, g_physical_address physicalAddress, uint32_t globalSystemInterruptBase, g_ioapic* next) :
			id(id), physicalAddress(physicalAddress), virtualAddress(0), globalSystemInterruptBase(globalSystemInterruptBase), redirectEntryCount(0), next(next) {
	}
