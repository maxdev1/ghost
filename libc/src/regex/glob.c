/**
 * Original implementation taken from musl (/src/regex/glob.c)
 *
 * Modified & made C++ compatible for Ghost.
 */
#include <glob.h>
#include <fnmatch.h>
#include <sys/stat.h>
...
static int append(struct match **tail, const char *name, size_t len, int mark) {
	match* newMatch = (match*) malloc(sizeof(struct match) + len + 1);
	if (!newMatch) {
		return -1;
	}
	(*tail)->next = newMatch;
	newMatch->next = nullptr;
	strcpy(newMatch->name, name);
	if (mark) {
		strcat(newMatch->name, "/");
	}
	*tail = newMatch;
	return 0;
}
...
void globfree(glob_t *g) {
	...
	for (i = 0; i < g->gl_pathc; i++) {
		free(g->gl_pathv[g->gl_offs + i] - offsetof(match, name));
	}
	free(g->gl_pathv);
