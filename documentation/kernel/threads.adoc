...
Threads
-------
Threads hold information about their current state and their configuration.
All threads run solely in user-space, there are no threads running in
kernel-space for design reasons.

Each thread has a unique id, typed by `g_tid` (and `g_pid` respectively). This
id can be obtained from within the thread using `<<libapi#g_get_tid,g_get_tid>>`.

Each thread has its own userspace and kernelspace thread. See the
<<memory#Stacks,stack section>> of the memory layout documentation for details
on allocation and layout of these.
...
is used. This function must be supplied with an address pointing to code that will
be executed once the thread starts. Typically, a function name is used as the
execution entry. The variations of this function can also be supplied with a
pointer to user data. A simple example could look like this:
